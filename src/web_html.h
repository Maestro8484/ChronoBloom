...
static const char INDEX_P2[] PROGMEM =
  "<body><main>\n"
  "<section class='stage'>\n"
  "<h1>ESP32 Ring Clock <span id='fwVersion' style='font-size:11px;font-weight:400;color:var(--muted);vertical-align:middle'></span></h1>\n"
  ...
  "  <label style='margin-top:2px'>Clock-face brightness range. The light sensor dims the whole face between these two ends depending on how bright the room is. (Animations &amp; nudges have their own brightness &#8212; the &ldquo;Animation brightness&rdquo; slider in Motion &amp; nudges.)</label>\n"
  ...
  "  <label style='margin:5px 0 2px'>&#128262; Overall dimness &#8212; scales the whole auto response down. Lower = dimmer everywhere &amp; the sensor keeps directing brightness further into bright rooms.</label>\n"
  "  <div class='brightwrap'><span>&#128262;</span><input id='autoBrightnessGain' type='range' min='0' max='100'><output id='autoBrightnessGainOut'></output>%</div>\n"
  ...
  "<p class='sub' style='margin:0 0 4px'>A gentle nudge to look up and shift focus, at set intervals.</p>\n"
  ...
  "<div class='toggle'><label><input id='secondTrail' type='checkbox'>Second trail</label><label><input id='progressSeconds' type='checkbox'>Progress ring</label><label><input id='hourlyChime' type='checkbox'>Hourly bloom</label><label><input id='statusAnimations' type='checkbox'>Status</label></div>\n"
  ...
  "<div class='ringrow'><span>Petal texture</span><label style='display:flex;align-items:center;gap:6px;cursor:pointer'><input id='petalMode' type='checkbox'> scalloped petal fills on rings 1 &amp; 2</label></div>\n"
  ...
  "<div><label>Ring rotation offset (0-59 LEDs)</label><input id='outerRingOffset' type='number' min='0' max='59'></div></div>\n"
  "</div>\n"
  "</details>\n"
  "<details class='drawer'>\n"
  ...
  "<div class='row'><button onclick='post(\"/addMinute\")'>+1 min</button><button onclick='post(\"/subMinute\")'>-1 min</button><button onclick='syncBrowser()'>Sync to browser</button><button onclick='post(\"/syncNtp\")'>Sync to internet</button></div>\n"
  ...
  "</select></div>"
  "<div style='flex:3'><label>POSIX TZ string</label><input id='timezone' type='text' spellcheck='false' placeholder='MST7MDT,M3.2.0,M11.1.0'></div>"
  "<button onclick='saveTz()'>Save zone</button></div>\n"
...
static const char INDEX_P3[] PROGMEM =
  ...
  "function bindLive(){for(const k of ['outerMarkerLevel','outerFillerLevel','secondsLevel','minutesLevel','middleFaceScale','hoursLevel','innerFaceScale','innerHourLevel','centerLevel']){const out=qs(k+'Out');const upd=()=>{out.value=qs(k).value;if(['middleFaceScale','innerFaceScale','hoursLevel','innerHourLevel'].includes(k))qs('contrastPreset').value='';draw()};qs(k).oninput=upd;upd()}for(const k of ['outerMarkerColor','outerFillerColor','secondsColor','minutesColor','centerColor','previewMode'])qs(k).oninput=draw;for(const k of ['middleFaceColor','hoursColor','innerFaceColor','innerHourColor'])qs(k).oninput=()=>{qs('contrastPreset').value='';draw()};for(const k of ['secondTrail','progressSeconds'])qs(k).oninput=()=>{settings[k]=qs(k).checked;draw()};for(const k of ['animationBrightness','trailLength']){const out=qs(k+'Out');const upd=()=>{out.value=qs(k).value};qs(k).oninput=upd;upd()}for(const k of ['secondTrailLength','progressLevel']){const out=qs(k+'Out');const upd=()=>{out.value=qs(k).value;draw()};qs(k).oninput=upd;upd()}for(const k of ['secondTrailStyle','progressStyle'])qs(k).oninput=draw;qs('outerRingBrightness').oninput=()=>{qs('contrastPreset').value='';draw()};qs('petalMode').oninput=draw;for(const k of ['dayBrightness','minAutoBrightness','maxAutoBrightness','autoBrightnessGain']){const o=qs(k+'Out');if(o){const u=()=>{o.value=qs(k).value};qs(k).oninput=u;u()}}}\n"
  ...
  "function tzPicked(){const v=qs('tzPick').value;if(v!=='__custom')qs('timezone').value=v;qs('timezone').focus()}\n"
  "function tzSync(){const v=qs('timezone').value.trim();const opt=[...qs('tzPick').options].find(o=>o.value===v);qs('tzPick').value=opt?v:'__custom'}\n"
  ...
  "async function saveTz(){const v=qs('timezone').value.trim();const msg=qs('tzMsg');"
  "const r=await fetch('/settings',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'timezone='+encodeURIComponent(v)+'&silent=1'});"
  "if(r.ok){msg.textContent='Saved. Clock is now on '+v;tzSync();await refresh()}"
  ...
  "function previewAnim(type,modeId){const mode=qs(modeId).value;if(!mode||mode==='0')return;const p=styleParams(new URLSearchParams());p.set('type',type);p.set('mode',mode);fetch('/previewAnimation',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:p.toString()})}\n"
  ...
  "function saveBrightness(){const p=new URLSearchParams();for(const k of ['autoBrightnessMode','dayBrightness','minAutoBrightness','maxAutoBrightness','autoBrightnessGain'])p.set(k,qs(k).value);post('/settings',p.toString()).then(loadSettings)}\n"
  ...
  "async function updateDemoStatus(){try{const r=await fetch('/demo/status');const data=await r.json();if(data.active){const elapsed=data.elapsed_ms,dur=data.step_duration_ms,pct=Math.round(100*elapsed/dur);qs('demoStatus').textContent=`Step ${data.step+1}/${data.steps||9}: ${data.subtitle} (${pct}%)`}else{qs('demoStatus').textContent='Idle'}}catch(e){}}\n"
  ...
  "const THEMES={\"chronobloom\":{\"name\":\"ChronoBloom (clock default)\",\"version\":3,\"outerRingBrightness\":77,\"outerMarkerColor\":\"#6eb9ff\",\"outerFillerColor\":\"#0008c8\",\"secondsColor\":\"#64ffb4\",\"minutesColor\":\"#ff6400\",\"middleFaceColor\":\"#dc00b4\",\"hoursColor\":\"#dc00b4\",\"innerFaceColor\":\"#ff3c00\",\"innerHourColor\":\"#dc00b4\",\"centerColor\":\"#ff3c00\",\"petalMode\":0,\"outerMarkerLevel\":225,\"outerFillerLevel\":145,\"secondsLevel\":230,\"minutesLevel\":220,\"middleFaceScale\":55,\"hoursLevel\":255,\"innerFaceScale\":55,\"innerHourLevel\":255,\"centerLevel\":180,\"animationPalette\":7,\"animationSpeed\":3,\"animationBrightness\":157,\"trailLength\":6,\"reminderPalette\":0},\"moonflower\":{\"name\":\"Moonflower (night bloom)\",\"version\":3,\"outerRingBrightness\":70,\"outerMarkerColor\":\"#c8d8ff\",\"outerFillerColor\":\"#002974\",\"secondsColor\":\"#c8fff0\",\"minutesColor\":\"#b478ff\",\"middleFaceColor\":\"#863cd9\",\"hoursColor\":\"#f0f0ff\",\"innerFaceColor\":\"#bfa8e0\",\"innerHourColor\":\"#f0f0ff\",\"centerColor\":\"#ffc864\",\"petalMode\":1,\"outerMarkerLevel\":210,\"outerFillerLevel\":120,\"secondsLevel\":215,\"minutesLevel\":220,\"middleFaceScale\":50,\"hoursLevel\":255,\"innerFaceScale\":55,\"innerHourLevel\":255,\"centerLevel\":170,\"animationPalette\":7,\"animationSpeed\":2,\"animationBrightness\":150,\"trailLength\":6,\"reminderPalette\":1},\"cherryblossom\":{\"name\":\"Cherry Blossom (spring bloom)\",\"version\":3,\"outerRingBrightness\":75,\"outerMarkerColor\":\"#ffb4c8\",\"outerFillerColor\":\"#662d3f\",\"secondsColor\":\"#64ff8c\",\"minutesColor\":\"#ffa050\",\"middleFaceColor\":\"#ff3c78\",\"hoursColor\":\"#ff3c78\",\"innerFaceColor\":\"#ff7850\",\"innerHourColor\":\"#ff3c78\",\"centerColor\":\"#ffb428\",\"petalMode\":1,\"outerMarkerLevel\":220,\"outerFillerLevel\":130,\"secondsLevel\":220,\"minutesLevel\":215,\"middleFaceScale\":50,\"hoursLevel\":255,\"innerFaceScale\":55,\"innerHourLevel\":255,\"centerLevel\":180,\"animationPalette\":7,\"animationSpeed\":3,\"animationBrightness\":157,\"trailLength\":6,\"reminderPalette\":2},\"emberdahlia\":{\"name\":\"Ember Dahlia (fire bloom)\",\"version\":3,\"outerRingBrightness\":78,\"outerMarkerColor\":\"#ffc83c\",\"outerFillerColor\":\"#970000\",\"secondsColor\":\"#fff0c8\",\"minutesColor\":\"#ff5000\",\"middleFaceColor\":\"#ff7000\",\"hoursColor\":\"#ff1e00\",\"innerFaceColor\":\"#ffe25a\",\"innerHourColor\":\"#ff1e00\",\"centerColor\":\"#ffe07a\",\"petalMode\":1,\"outerMarkerLevel\":225,\"outerFillerLevel\":135,\"secondsLevel\":225,\"minutesLevel\":220,\"middleFaceScale\":55,\"hoursLevel\":255,\"innerFaceScale\":60,\"innerHourLevel\":255,\"centerLevel\":190,\"animationPalette\":7,\"animationSpeed\":3,\"animationBrightness\":160,\"trailLength\":5,\"reminderPalette\":0},\"lotuspond\":{\"name\":\"Lotus Pond (water lily)\",\"version\":3,\"outerRingBrightness\":76,\"outerMarkerColor\":\"#a7e0d8\",\"outerFillerColor\":\"#2d7a7f\",\"secondsColor\":\"#3cff96\",\"minutesColor\":\"#ffb43c\",\"middleFaceColor\":\"#ffd1df\",\"hoursColor\":\"#ff50b4\",\"innerFaceColor\":\"#f36464\",\"innerHourColor\":\"#ff50b4\",\"centerColor\":\"#ffc850\",\"petalMode\":1,\"outerMarkerLevel\":220,\"outerFillerLevel\":140,\"secondsLevel\":225,\"minutesLevel\":215,\"middleFaceScale\":50,\"hoursLevel\":255,\"innerFaceScale\":55,\"innerHourLevel\":255,\"centerLevel\":180,\"animationPalette\":7,\"animationSpeed\":3,\"animationBrightness\":157,\"trailLength\":6,\"reminderPalette\":2}};\n"
  "THEMES.sunflower={name:'Sunflower (gold & rust)',version:3,outerRingBrightness:82,outerMarkerColor:'#ff9500',outerFillerColor:'#ffa200',secondsColor:'#af0000',minutesColor:'#b40000',middleFaceColor:'#ff7300',hoursColor:'#b30000',innerFaceColor:'#ffa000',innerHourColor:'#b40404',centerColor:'#660014',petalMode:0,outerMarkerLevel:28,outerFillerLevel:248,secondsLevel:163,minutesLevel:220,middleFaceScale:181,hoursLevel:255,innerFaceScale:244,innerHourLevel:255,centerLevel:195,animationPalette:7,animationSpeed:3,animationBrightness:165,trailLength:6,reminderPalette:0};\n"
  "THEMES.birdofparadise={name:'Bird of Paradise (orange & cobalt)',version:3,outerRingBrightness:82,outerMarkerColor:'#ff5a0a',outerFillerColor:'#0e28dc',secondsColor:'#fff2dc',minutesColor:'#ff6e14',middleFaceColor:'#446b81',hoursColor:'#ff7014',innerFaceColor:'#206637',innerHourColor:'#fff2dc',centerColor:'#ff5a00',petalMode:1,outerMarkerLevel:225,outerFillerLevel:135,secondsLevel:210,minutesLevel:220,middleFaceScale:95,hoursLevel:255,innerFaceScale:90,innerHourLevel:255,centerLevel:205,animationPalette:7,animationSpeed:3,animationBrightness:165,trailLength:6,reminderPalette:0};\n"
  ...
  "async function saveAsTheme(){const name=(prompt('Save current clock as a theme - name it:')||'').trim();if(!name)return;const r=await fetch('/settings');const s=await r.json();delete s.hasUserDefaults;const o=customThemes();o[name]=s;setCustomThemes(o);renderThemeChips();qs('themeStatus').textContent='Saved theme: '+name}\n"
  ...
  "function applyTheme(key){if(!key)return;const t=THEMES[key];if(!t)return;const fields=['outerMarkerColor','outerFillerColor','secondsColor','minutesColor','middleFaceColor','hoursColor','innerFaceColor','innerHourColor','centerColor'];const levels=['outerMarkerLevel','outerFillerLevel','secondsLevel','minutesLevel','middleFaceScale','hoursLevel','innerFaceScale','innerHourLevel','centerLevel'];const anim=['animationPalette','animationSpeed','animationBrightness','trailLength','reminderPalette'];for(const k of fields)if(t[k])qs(k).value=t[k];for(const k of levels)if(t[k]!=null)qs(k).value=t[k];for(const k of anim)if(t[k]!=null)qs(k).value=t[k];if(t.outerRingBrightness!=null)qs('outerRingBrightness').value=t.outerRingBrightness;qs('petalMode').checked=!!t.petalMode;draw();saveSettings();saveAnimStyle();qs('themeStatus').textContent='Theme applied: '+t.name}\n"
