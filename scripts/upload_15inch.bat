@echo off
REM Build and OTA-upload ChronoBloom firmware to the 15" clock.
REM Usage: double-click, or run from any directory.

setlocal
set REPO_ROOT=%~dp0..
set ENV_NAME=esp32c3_v3_15inch
set HOSTNAME=esp32c3-v3-15inch.local
set FIRMWARE_BIN=%REPO_ROOT%\.pio\build\%ENV_NAME%\firmware.bin

echo ==========================================================
echo  ChronoBloom - 15inch build + OTA upload
echo ==========================================================
...
if errorlevel 1 (
    ...
    echo   - USB fallback: pio run -e %ENV_NAME% -t upload --upload-protocol esptool --upload-port COM9
    pause
    exit /b 1
)

echo.
echo Done. 15inch clock updated.
pause
